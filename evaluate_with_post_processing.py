...
def predict_segmentation(subject_name,
    ...
    with tf.Graph().as_default():
        ...
        _, pred_seg_softmax_3d_noisy_autoencoded_softmax, _ = model.predict_l2l(predicted_seg_1hot_3d_pl,
                                                                                exp_config,
                                                                                training_pl = tf.constant(False, dtype=tf.bool))
        ...
        for b_i in range(0, image.shape[0], 1):
            ...
            mask_predicted.append(sess.run(predicted_seg, feed_dict={images_pl: X}))
        ...
        mask_predicted_soft = np.squeeze(np.array(mask_predicted_soft)).astype(float)  
        ...
        if post_process is True:
            ...
            mask_predicted_soft_downsampled = rescale(mask_predicted_soft,
                                                      ...
                                                      multichannel=True,
                                                      mode='constant')
            mask_predicted_downsampled = np.argmax(mask_predicted_soft_downsampled, axis=-1)
            ...
            for _ in range(exp_config.dae_post_process_runs):
                feed_dict = {predicted_seg_3d_pl: np.expand_dims(mask_predicted_denoised, axis=0)}      
                y_pred_noisy_denoised_softmax = np.squeeze(sess.run(pred_seg_softmax_3d_noisy_autoencoded_softmax, feed_dict=feed_dict)).astype(np.float16)               
                mask_predicted_denoised = np.argmax(y_pred_noisy_denoised_softmax, axis=-1)
...
def main():
    ...
    if exp_config.post_process is True:
        file_suffix = '_with_post_process_with_dae_runs' + str(exp_config.dae_post_process_runs)
    else:
        file_suffix = ''
    ...
    # ================================       
    results_file = open(log_dir + '/' + test_dataset_name + '_' + 'test' + file_suffix + '.txt', "w")
    results_file.write("================================== \n") 
    ...
    for sub_num in range(num_test_subjects): 
        ...
        # ==================================================================    
        hsd_per_label_this_subject = utils.compute_surface_distance(y1 = labels_orig, y2 = predicted_labels_orig_res_and_size, nlabels = exp_config.nlabels)
        ...
        utils_vis.save_sample_prediction_results(x = utils.crop_or_pad_volume_to_size_along_z(image_orig, 256),
                                                 ...
                                                 savepath = log_dir + '/' + test_dataset_name + '_' + 'test' + '_' + subject_name + file_suffix + '.png')
