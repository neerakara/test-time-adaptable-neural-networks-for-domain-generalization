...
def main():
    ...
    for sub_num in range(5): 
        ...
        z_pred = utils.crop_or_pad_volume_to_size_along_z(predicted_labels_orig_res_and_size, 256)
        ...
        for zz in np.arange(120, 130, 10):
            ...
            utils_vis.save_single_image(x_norm[:,:,zz], basepath + 'slice' + str(zz) + '_x_norm.png', 15, False, 'gray', False, climits = [-1.0, 2.0])
            utils_vis.save_single_image(z_true[:,:,zz], basepath + 'slice' + str(zz) + '_z_true.png', 15, True, 'tab20', False)
            utils_vis.save_single_image(z_pred[:,:,zz], basepath + 'slice' + str(zz) + '_z_pred.png', 15, True, 'tab20', False)
