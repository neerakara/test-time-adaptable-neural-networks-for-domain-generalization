...
def run_training(log_dir,
    ...
    with tf.Graph().as_default():
        ...
        accumulated_gradients = [tf.Variable(tf.zeros_like(var.initialized_value()), trainable=False) for var in i2l_vars_trainable]
        ...
        with tf.control_dependencies(update_ops):
            gradients = optimizer.compute_gradients(loss_op, var_list = i2l_vars_trainable)
        ...
        while (step < max_steps_tta):
            ...
            if (step == init_step) or (step % exp_config.check_ood_frequency is 0): 
                ...
                if exp_config.use_gt_for_tta is True:
                    ...
                    prr = dBgt                    
                elif (ratio_dice > exp_config.dae_atlas_ratio_threshold) and (dAC > exp_config.min_atlas_dice):
...
def main(argv):
    ...
    for subject_id in range(subject_num, subject_num+1):
        ...
        if subject_id != first_subject:
            log_dir_first_TD_subject = log_dir_base + '/subject_' + str(name_test_subjects[first_subject])[2:-1]
